...
calibrate<-function(wd,infrared.data,reference.data,method="RF"){
	if(method=="RF"){
#a<-wd#Mac OSX
#a<-"D:/oaf"#Windows
setwd(wd)
...
cb<-0.8*mean(range(y))
bb<-0.85*mean(range(y))
rfp<-as.data.frame(cbind(y,predi))#get back to original scale
...
colnames(msummary)<-c("Soil properties","PCs","OOB RMSEC","OOB Rsquared", "Holdout RMSEP","Holdout Rsquared")
write.table(msummary,file="Model_Summary.csv",sep=",",row.names=FALSE)
