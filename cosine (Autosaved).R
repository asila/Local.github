...
write.table(mir.cos,file="Cosine angle for Afsis samples.csv",sep=",",row.names=TRUE)
...
#pick sfi for all the 10 pure minerals
pnm<-colnames(mir.cos[,1:10])

s.min<-NULL
for (k in 1:length(pnm)){
sk<-which(colnames(mir.cos)==pnm[k])
sf<-mir.cos[,sk]
s.min<-cbind(s.min,sf)}
sfi<-as.data.frame(cbind(as.vector(colnames(mir.cos)),mir.cos[,s1],s.min))[-c(s1,1:10),]
colnames(sfi)<-c("SSN","SFI.humus",paste0("SFI.",pnm))
...
lab.sfi<-merge(afsis.lab,sfi,by="SSN")
...
sfi.pca<-PCA(labs[,c(1:17,32:34,47,48,min)],scale.unit=TRUE,ind.sup=labs[,1])
labs$ssfi1

codes<-read.csv("~/Dropbox/AfSIS_reporting_data/Seperated_datasets/Ldsf_field_details_SSN.csv")

sfic<-merge(codes,sfi,by="SSN")
write.table(sfic,file="~/Thesis/Data/AfSIS_labdata sfi ldsf codes.csv",sep=",",row.names=FALSE)
library(lattice)
xyplot(sfic$SFI.quartz~sfic$Country)