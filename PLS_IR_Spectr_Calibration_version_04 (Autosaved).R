...
results.folder<-"~/Thesis/data_fusion/Montmorillonite"

#Read into R both IR and chem data

raw<-read.csv("~/Dropbox/AfSIS_reporting_data/Seperated_datasets/Calibration_Htsxt_MIR.csv")
spec.class<-read.csv("~/Thesis/cosine/AfSIS spec classes.csv")
#Remove sample with ssn "icr075956"; this is AfSIS sample with high CN 
spec.raw<-merge(spec.class,raw)
endme<-unique(spec.raw$endmember.ssn)
raw<-subset(spec.raw,spec.raw$endmember.ssn==endme[1])[,-2]
nraw<-names(raw)
...
chem<-read.csv("~/Dropbox/AfSIS_reporting_data/Seperated_datasets/AfSIS reference data.csv")
summary(chem)
##################################################################
