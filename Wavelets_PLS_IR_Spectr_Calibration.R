...
#set der to the ir table with wavelets transformed
der<-raw
wtc<-colnames(chem)
#################################################
...
c<-ncol(chem)
wavelet_transformed_coefficients<-colnames(calibset)
#y<-select.list(as.character(wavelet_transformed_coefficients),graphics=TRUE,title="Select region where MIR begins and ends",multiple=TRUE)

wc<-which(wavelet_transformed_coefficients%in%colnames(der[,-1]))

cutspec<-as.matrix(calibset[,wc])
...
ifelse(jj>0,res.pls<-mvr_dcv(ref~.,data=cutspe,plot.opt=FALSE,ncomp=p,method="widekernelpls",repl=1,selstrat="hastie",segments0=4,segment0.type="random",na.action=na.omit,segments=4,segment.type="random"),"Do nothing")
...
#Run the best model
mvr.tn<-plsr(na.omit(as.numeric(cutchem[,q])~cutspec,ncomp=nc,validation ="CV"))
...
a<-round(min(na.omit(as.numeric(cutchem[,q])),na.omit(mvr.tn$fitted.values[,,nc])),1)

am<-round(max(na.omit(as.numeric(cutchem[,q])),na.omit(mvr.tn$fitted.values[,,nc])),1)
...
plot(na.omit(as.numeric(cutchem[,q]))~na.omit(mvr.tn$fitted.values[,,nc]),pch=19,col="blue",ylab="",xlab="",main="",ylim=c(a,b),xlim=c(a,b))
bl<-lm(na.omit(as.numeric(cutchem[,q]))~mvr.tn$fitted.values[,,nc])
...
#Calculate residuals
resid<-na.omit(as.numeric(cutchem[,q]))-na.omit(mvr.tn$fitted.values[,,nc])
...
wavl<-as.numeric(substr(colnames(cutspec),4,19))
